...
impl Resolver {
    ...
    fn resolve_platform_by_board(&self, mandatory_target_resolution: bool) -> Result<Resolution> {
        ...
        if let Some(configured_mcu) = params.mcu.as_ref() {
            // PATCHED (MicroAgent): compare case-insensitively because the ESP32-C61
            // MCU is sometimes uppercase ("ESP32C61" in `pio boards --json-output`)
            // and sometimes lowercase ("esp32c61" in `.cargo/config.toml`). Without
            // this, embuild bails with a non-helpful "Platforms mismatch" error
            // when the case doesn't match exactly.
            if !configured_mcu.eq_ignore_ascii_case(&board.mcu) {
                bail!(
    ...
    pub fn derive_target_conf(target: impl AsRef<str>) -> Result<TargetConf> {
        Ok(match target.as_ref() {
            ...
            "riscv32imac-unknown-none-elf" | "riscv32imac-esp-espidf" => TargetConf {
                platform: "espressif32",
                // PATCHED (MicroAgent): added ESP32C61 — see Cargo.toml patch notes.
                mcus: vec!["ESP32C6", "ESP32H2", "ESP32C5", "ESP32P4", "ESP32C61"],
                frameworks: vec!["espidf", "arduino"],
            },
    ...
    pub fn derive_target(mcu: impl AsRef<str>) -> Result<&'static str> {
        ...
        } else if mcu == "esp32h2" || mcu == "esp32c5" || mcu == "esp32c6" || mcu == "esp32p4" || mcu == "esp32c61" {
            // ESP32CX with atomics support
            // PATCHED (MicroAgent): Added esp32c61 (ESP32-C61 uses same arch as C6)
