        ...
        let sys_config = gptimer_config_t {
            ...
            flags: gptimer_config_t__bindgen_ty_1 {
                _bitfield_1: gptimer_config_t__bindgen_ty_1::new_bitfield_1(
                    ...
                    // PATCHED (MicroAgent): ESP-IDF v6 removed
                    // `backup_before_sleep` from `gptimer_config_t`. The
                    // `not(esp_idf_version_at_least_6_1_0)` gate in the
                    // upstream code is incorrect for v6.0.x, which is
                    // `>= 6.1.0` semantically but not actually present.
                    // We exclude this field entirely for v6.0+.
                    #[cfg(all(
