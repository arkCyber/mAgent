...
pub mod config {
    ...
    impl From<&UnitConfig> for pcnt_unit_config_t {
        fn from(value: &UnitConfig) -> Self {
            pcnt_unit_config_t {
                // PATCHED (MicroAgent): ESP-IDF v6 added a `clk_src` field
                // to `pcnt_unit_config_t`. ESP-IDF 5.x didn't have it; the
                // bindgen-generated bindings.rs now contains it for v6+.
                // We default to `PCNT_CLK_SRC_DEFAULT` (0), which is the
                // same default the C driver uses when `clk_src` is left
                // unset.
                // PATCHED (MicroAgent): clk_src was removed in ESP-IDF v6.0.
                // The C struct `pcnt_unit_config_t` no longer has the
                // `clk_src` field; PCNT now derives the clock source
                // internally from the SOC root clock. Drop the
...
pub struct PcntUnitDriver<'d> {
    ...
    // PATCHED (MicroAgent): ESP-IDF v6 removed `SOC_PCNT_CHANNELS_PER_UNIT`
    // from the public SOC caps (it was a kconfig macro that bindgen can't
    // always resolve). The PCNT peripheral on ESP32-C6/C61 has 2 channels
    // per unit. We hardcode it here. (For other targets, this may need to
    // be adjusted — ESP32 has 2, ESP32-S2 has 4, etc.)
    channels: Vec<PcntChannel<'d>, 2>,
    #[cfg(feature = "alloc")]
