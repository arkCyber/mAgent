        ...
        T: Borrow<SpiDriver<'d>>,
    {
        ...
        pub fn new_spi(
            ...
            let configuration = sdmmc_host_t {
                ...
                // PATCHED (MicroAgent): ESP-IDF v6 added a
                // `unaligned_multi_block_rw_max_chunk_size` field. ESP-IDF
                // 5.x didn't have it; the bindgen-generated bindings.rs
                // now contains it for v6+. We default to 0 (no
                // restriction), which matches the upstream driver's
                // initialization logic.
                #[cfg(esp_idf_version_at_least_6_0_0)]
                unaligned_multi_block_rw_max_chunk_size: 0,
                // PATCHED (MicroAgent): The cfg above was apparently
                // not triggered in the bindings.rs we just generated,
                // so the field is *always* present. Drop the gate.
                unaligned_multi_block_rw_max_chunk_size: 0,
            };
