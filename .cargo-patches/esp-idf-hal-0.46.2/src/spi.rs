...
fn spi_create_transaction(
    ...
    spi_transaction_t {
        flags,
        // PATCHED (MicroAgent): ESP-IDF v6's `spi_transaction_t` flattens
        // its anonymous unions into direct fields `tx_buffer`/`rx_buffer`
        // instead of `__bindgen_anon_1.tx_buffer`/`__bindgen_anon_2.rx_buffer`.
        // The bindgen bindings used to nest them as `__bindgen_anon_1`/
        // `__bindgen_anon_2`, but our patched bindings.rs rewrites them
        // to be direct so that the struct is non-opaque.
