...
impl RawCondvar {
    ...
    pub unsafe fn wait_timeout(&self, mutex: &RawMutex, duration: Duration) -> bool {
        ...
        let abstime = timespec {
            // PATCHED (MicroAgent): cast each operand to i64 first
            // because ESP-IDF v6 narrowed `time_t` from i64 to i32 on
            // 32-bit targets (RISC-V and Xtensa). The original code
            // tried to widen `as_secs()` (u64) into `time_t` (now i32)
            // and then add it to `now.tv_sec` (i32), which produced
            // an `i32 + i64` type mismatch.
            tv_sec: (now.tv_sec as i64 + duration.as_secs() as i64) as i64,
            // PATCHED (MicroAgent): tv_nsec is i32 on the RISC-V espidf target
            // (`struct timespec { tv_sec: i64, tv_nsec: i32 }` — `long` is
            // 32-bit on this target). Calculate in i64 then cast back to i32.
            tv_nsec: (((now.tv_usec as i64) * 1000 + duration.subsec_nanos() as i64) as i32),
        };
