    ...
    pub fn next_key(&mut self) -> Option<(&str, NvsDataType)> {
        ...
        match unsafe { nvs_entry_info(self.raw_iter, &mut info as *mut _) } {
            ...
            ESP_OK => {
                ...
                // PATCHED (MicroAgent): Cast key from i8 to u8 since copy_from_slice expects u8
                // Use from_raw_parts to reinterpret the slice without copying
                let key_u8: &[u8] = unsafe {
                    std::slice::from_raw_parts(info.key.as_ptr() as *const u8, info.key.len())
                };
                self.key_name_buffer[..info.key.len()].copy_from_slice(key_u8);
