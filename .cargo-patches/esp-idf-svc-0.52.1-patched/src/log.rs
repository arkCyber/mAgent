...
impl EspStdout {
    fn new() -> Self {
        ...
        #[cfg(not(esp_idf_libc_picolibc))]
        {
            ...
            // PATCHED (MicroAgent): Always use picolibc API since we enable picolibc mode
            // Copied from here:
            ...
            unsafe {
                __retarget_lock_acquire_recursive(file.lock);
            }
        }
...
impl Drop for EspStdout {
    fn drop(&mut self) {
        ...
        // PATCHED (MicroAgent): Always use picolibc API since we enable picolibc mode
        unsafe {
            __retarget_lock_release_recursive(file.lock);
        }
    }
}
...
impl core::fmt::Write for EspStdout {
    fn write_str(&mut self, s: &str) -> core::fmt::Result {
        ...
        unsafe {
            fwrite(slice.as_ptr() as *const _, 1, slice.len() as usize, self.0);
        }
