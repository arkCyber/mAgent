...
// PATCHED (MicroAgent): also fall back to a hardcoded value for
// ESP32-C6/C61 on ESP-IDF v6.x, where `SOC_PCNT_UNITS_PER_GROUP` is not
// emitted into `bindings.rs` (the soc_caps.h constant does exist but is
// guarded by a `__has_include` check that bindgen doesn't resolve).

/// Maximum number of PCNT units
#[cfg(not(any(
    all(esp32h2, esp_idf_version_major = "4"),
    esp32c6,
    esp32c61
)))]
...
/// PATCHED (MicroAgent): ESP32-C6 has 1 PCNT unit per group, 1 group.
#[cfg(esp32c6)]
pub const pcnt_unit_t_PCNT_UNIT_MAX: pcnt_unit_t = 1;

/// PATCHED (MicroAgent): ESP32-C61 (our target) has 1 PCNT unit per group, 1 group.
#[cfg(esp32c61)]
pub const pcnt_unit_t_PCNT_UNIT_MAX: pcnt_unit_t = 1;
