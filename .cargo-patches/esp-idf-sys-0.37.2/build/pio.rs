...
pub fn build() -> Result<EspIdfBuildOutput> {
    ...
    let (pio_scons_vars, link_args, config) =
        if let Some(pio_scons_vars) = project::SconsVariables::from_piofirst() {
            ...
            // PATCHED (MicroAgent): Use MCU to derive the correct cross-compilation target
            // instead of using the host TARGET env var which returns "aarch64-apple-darwin"
            let target = if let Some(ref mcu) = config.mcu {
                let mcu_lower = mcu.to_lowercase();
                if mcu_lower.starts_with("esp32c6") || mcu_lower.starts_with("esp32c5") 
                    || mcu_lower.starts_with("esp32h2") || mcu_lower.starts_with("esp32c61")
                    || mcu_lower.starts_with("esp32p4") {
                    "riscv32imac-esp-espidf".to_string()
                } else if mcu_lower.starts_with("esp32c3") || mcu_lower.starts_with("esp32c2") {
                    "riscv32imc-esp-espidf".to_string()
                } else if mcu_lower.starts_with("esp32s3") {
                    "xtensa-esp32s3-espidf".to_string()
                } else if mcu_lower.starts_with("esp32s2") {
                    "xtensa-esp32s2-espidf".to_string()
                } else {
                    // Default to ESP32
                    "xtensa-esp32-espidf".to_string()
                }
            } else {
                env::var("TARGET").unwrap_or_else(|_| "riscv32imac-esp-espidf".to_string())
            };
            ...
            let mut link_args_builder = build::LinkArgsBuilder::try_from(&pio_scons_vars)?;
            // PATCHED (MicroAgent): force the underlying linker to be
            // `riscv32-esp-elf-gcc` so we don't go through `ldproxy`.
            // `ldproxy` requires a `--ldproxy-linker <linker>` argument
            // which `pio.rs` doesn't pass; without it the linker exits
            // with "Cannot locate argument '--ldproxy-linker <linker>'".
            // The default RUSTC_LINKER is `ldproxy` from
            // .cargo/config.toml; we override to `riscv32-esp-elf-gcc`
            // here so `cargo:rustc-linker=...` is set for downstream
            // crates too.
            link_args_builder = link_args_builder.linker(std::path::PathBuf::from("riscv32-esp-elf-gcc"));
            let link_args = link_args_builder.build()?;
    ...
    let esp_idf = PathBuf::from(pio_scons_vars.pio_framework_dir.as_deref().unwrap_or(""));
