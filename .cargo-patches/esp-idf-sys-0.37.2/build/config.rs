...
impl BuildConfig {
    ...
    pub fn try_from_env() -> Result<BuildConfig> {
        // PATCHED (MicroAgent): Debug output for environment variables
        eprintln!("DEBUG: TARGET env = {:?}", std::env::var("TARGET").ok());
        eprintln!("DEBUG: MCU env = {:?}", std::env::var("MCU").ok());
        
        let cfg: BuildConfig = utils::parse_from_env(&[])?;
        eprintln!("DEBUG: parsed mcu = {:?}", cfg.mcu);
        ...
        // PATCHED (MicroAgent): If MCU is not set, try to derive it from TARGET
        // This is needed when building from a host machine that doesn't have MCU set
        let cfg = if cfg.mcu.is_none() {
            let target = std::env::var("TARGET").unwrap_or_default();
            let mcu = if target.contains("riscv32imac") || target.contains("riscv32imc") {
                // RISC-V ESP32 variants
                if target.contains("espidf") {
                    // PATCHED (MicroAgent): chip is ESP32-C61; default to esp32c61
                    // for RISC-V ESP-IDF targets. (Upstream hard-codes esp32c6 which
                    // makes PlatformIO compile a C6 bootloader for a C61 chip.)
                    Some("esp32c61".to_string())
                } else {
                    None
                }
            } else {
