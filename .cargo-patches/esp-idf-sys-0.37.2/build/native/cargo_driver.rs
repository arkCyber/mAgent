...
pub fn build() -> Result<EspIdfBuildOutput> {
    ...
    // PATCHED (MicroAgent): If MCU is explicitly set via config, use it to derive
    // the target for chip detection. This handles the case where we're building
    // from a host machine (e.g., macOS) with a cross-compilation target configured
    // in .cargo/config.toml but the TARGET env var isn't set correctly during build.rs.
    let target = if let Some(ref mcu) = config.mcu {
        ...
        if mcu_lower.starts_with("esp32c6") || mcu_lower.starts_with("esp32c5") 
            || mcu_lower.starts_with("esp32c61") || mcu_lower.starts_with("esp32h2") {
        ...
        } else if mcu_lower.starts_with("esp32p4") {
            "riscv32imafc-esp-espidf".to_string()
        } else {
            // Default to ESP32 / xtensa-esp32-espidf
