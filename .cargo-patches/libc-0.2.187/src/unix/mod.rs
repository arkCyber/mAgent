...
// PATCHED (MicroAgent): Implement Send + Sync for DIR on espidf target.
// This is required because fs_at crate (used by std's filesystem operations)
// requires DIR to be Send + Sync for thread-safe filesystem access.
// On ESP-IDF, DIR operations are inherently thread-safe as they use
// the underlying Picolibc/newlib which is designed for single-threaded or
// properly synchronized environments.
