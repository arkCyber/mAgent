...
//! control packets for the `magent-mqtt-ingest` example:
//!
...
//!
//! Originally lifted from `examples/mqtt-roundtrip/src/protocol.rs`
//! so this example stays self-contained; the two brokers / codecs
//! are intentionally kept identical so a regression in the wire
//! protocol surfaces in both.
...
impl std::fmt::Display for CodecError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            CodecError::UnknownPacketType(b) => {
                write!(f, "unknown packet type byte {:#x}", b)
            }
