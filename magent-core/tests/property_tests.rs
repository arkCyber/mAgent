...
mod secp256k1_tests {
    use super::*;
    use magent_core::web3::blockchain::{Secp256k1Keypair, Address};
...
fn skills_best_k_respects_k_param() {
    ...
    for k in 0..=15 {
        ...
        assert!(best.len() <= k, "best_k({}) returned {} items", k, best.len());
    }
}
...
fn skills_count_by_category_sums_to_total() {
    ...
    assert_eq!(total, mgr.count(),
        "sum of category counts {} must equal total {}", total, mgr.count());
}
